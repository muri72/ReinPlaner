...
- Name: complete.spec.ts >> 👷 Employees >> should have add employee button
- Location: e2e/complete.spec.ts:244:7
...
Locator: getByRole('link', { name: /neue.*mitarbeiter|employee.*add|mitarbeiter.*hinzufügen/i }).or(getByRole('button', { name: /\+.+|neu/i })).first()
Expected: attached
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('link', { name: /neue.*mitarbeiter|employee.*add|mitarbeiter.*hinzufügen/i }).or(getByRole('button', { name: /\+.+|neu/i })).first()
