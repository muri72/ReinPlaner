...
// FIX: Signatur vereinfacht, da prevState nicht mehr benötigt wird
export async function deleteTask(formData: FormData): Promise<{ success: boolean; message: string }> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
