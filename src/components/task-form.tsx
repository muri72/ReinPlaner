...
import { Button } from "@/components/ui/button"; // FIX: '=>' zu 'from' geändert
...
export function TaskForm({ initialData, onSubmit, submitButtonText, onSuccess }: TaskFormProps) {
  ...
  const resolvedDefaultValues: TaskFormValues = {
    ...
    dueDate: initialData?.dueDate ? new Date(initialData.dueDate) : undefined, // undefined statt null für optionales Datum
    status: initialData?.status ?? "pending", // status ist immer ein gültiger Enum-Wert
  };

  const form = useForm<TaskFormValues>({
    resolver: zodResolver(taskSchema as z.ZodSchema<TaskFormValues>), // Explizite Typumwandlung des Schemas
