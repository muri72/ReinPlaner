...
export function PlanningCalendar({ planningData, unassignedOrders, weekDays, activeDragId, showUnassigned, onActionSuccess, weekNumber }: PlanningCalendarProps) {
  ...
  // Mobile Ansicht - vereinfachte Darstellung
  if (isMobile) {
    return (
      <div className="border rounded-lg shadow-neumorphic glassmorphism-card h-full overflow-auto custom-scrollbar">
        <Table className="min-w-full border-collapse table-fixed">
          <TableHeader>
            <TableRow>
              <TableHead className="sticky left-0 bg-card z-20 w-[200px] text-xs">Mitarbeiter</TableHead>
              {weekDays.map((day) => (
                <TableHead key={day.toString()} className="text-center text-xs w-[80px] min-w-[80px]">
                  <div>{format(day, "E", { locale: de })}</div>
                  <div>{format(day, "dd.", { locale: de })}</div>
                </TableHead>
              ))}
            </TableRow>
          </TableHeader>
          <TableBody>
            ...
            {showUnassigned && (
              <TableRow>
                <TableCell className="font-semibold sticky left-0 bg-card z-10 align-top text-xs">
                  Unbesetzt
                </TableCell>
                {weekDays.map((day) => {
            ...
            {employeeIds.length === 0 ? (
              ...
              employeeIds.map(id => {
                ...
                return (
                  <TableRow key={id}>
                    <TableCell className="font-normal sticky left-0 bg-card z-10 align-top p-1 w-[200px]">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <EmployeeEditDialog employee={employee.raw as any} onEmployeeUpdated={onActionSuccess}>
                            <div className="text-xs font-semibold cursor-pointer hover:text-primary truncate">{employee.name}</div>
                          </EmployeeEditDialog>
                          <div className="text-xs text-muted-foreground">
                            {employee.totalHoursPlanned.toFixed(1)}/{employee.totalHoursAvailable.toFixed(1)}h
                          </div>
                        </div>
                      </div>
                    </TableCell>
                    {weekDays.map((day) => {
                      ...
                      if (!dayData) return <TableCell key={dateString} className="p-1"></TableCell>;
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div className={cn(
                                    "w-full h-full flex items-center justify-center font-semibold text-xs text-white rounded",
                      ...
                      return (
                        ...
                        >
                          <div className="space-y-1">
                            {dayData.assignments.slice(0, 2).map((assignment) => (
